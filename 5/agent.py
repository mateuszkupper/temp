...
class DQN_Agent:
    ...
    # Output: 		None
    def train(self):
        #self.load("models/save/data.chkp-220")
        while self.sess.run(self.episode) < self.training_metadata.num_episodes:
            ...
            if (episode % 10 == 0) and (episode != 0):
                ...
                if os.path.exists("5/CarRacing.txt"):
                    append_write = 'a' # append if already exists
                else:
                    append_write = 'w' # make a new file if not
                with open("5/CarRacing.txt", append_write) as f:
    ...
    def estimate_avg_q(self):
        if not self.q_grid:
            return 0
        return np.average(np.amax(self.sess.run(self.Q_value, feed_dict={self.state_tf: 
np.reshape(self.q_grid, (200, 32, 10, 1))}), axis=1))
